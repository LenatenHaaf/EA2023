...
def prop_selection(parent, parent_f):
    """
    Function performs proportional selection without a constant.

    Parameters:
        parent (list): list containing list of bit string of the parents.
        parent_f(list): fitness of the parent.

    Returns:
        parent1 (list): new parent created by proportional selection without a constant.
    """
    list_parents = []
...
def proportionalselection(parent, parent_f, c):
    """
    Function performs proportional selection with a constant.

    Parameters:
        parent (list): list containing list of bit string of the parents
        parent_f(list): fitness of the parent.
        c (float): added constant for the proportional selection.

    Returns:
        parent1 (list): new parent created by proportional selection with a constant.
    """
    offspring = []
    probability = []
    # c = min(parent_f) - 0.01
...
def tour_selection(parent, parent_f, k=5):
    """
    Function which performs tournament selection.

    Parameters:
        parent (list): list containing list of bit string of the parents.
        parent_f(list): fitness of the parent.
        k (int): the amount of samples chosen for the tournament selection.

    Returns:
        parent[indiv] (list): bit string containing information about the selected parent.
    """
    score = 0
    indiv = 0
...
def crossover(p1, p2, crossover_probability, n_points=2):
    """
    Function which is able to perform n-point crossover.

    Parameters:
        p1 (list): bit string of parent 1 without crossover
        p2 (list): bit string of parent 2 without crossover
        crossover_probability (float): the probability crossover will occur.
        n_points (int): The amount of crossover points in the bit strings.

    Returns:
        [p1, p2] (list): list containing the two new bit strings generated with crossover.
...
def uniform_crossover(p1, p2):
    """
    Function which is able to perform uniform crossover.
...
def mutation(p, mutation_rate):
    """
    Function performing the mutation with regard to the mutation rate.

    Parameters:
        p (list): bit string where mutation can occur.
        mutation_rate (float): the mutation rate the function needs to follow.
...
def s2566818_s4111753_GA(problem, selection_method, selection_size):
    """
    Function which performs the overall GA. Function is calling all the operations, for example the crossover and
    mutation. It keeps track of the budget as well and performs the initialization and the evaluation of the GA.

    Parameters:
        problem (ioh.iohcpp.problem.LABS): the problem imported by IOH (is either F18 or F19).
        selection_method (bool): The selection method chosen (is based on the test results and the most optimal values).
            - It is True for the F18 and False for F19.
        selection_size (bool): The selection size chosen (is based on the test results and the most optimal values).
            - It is True for the F18 and False for F19.
...
def create_problem(fid: int):
    ...
    l = logger.Analyzer(
        ...
        algorithm_name="GA_F18_F19",  # name of your algorithm
